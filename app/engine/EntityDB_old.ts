...
export class EntityDB<M extends Record<string, Entity>> {
  ...
  private createTables() {
    ...
    this.db.prepare("CREATE INDEX IF NOT EXISTS data_id ON data(id)").run();
    this.db.prepare("CREATE INDEX IF NOT EXISTS data_key ON data(key)").run();
    this.db
      .prepare("CREATE INDEX IF NOT EXISTS data_value ON data(value)")
      .run();
    this.db.prepare("CREATE INDEX IF NOT EXISTS data_type ON data(type)").run();
  }
  ...
  updateField<K extends keyof M, F extends keyof M[K]>(
    entity: M[K],
    key: keyof M[K],
    value: any,
    transaction = true
