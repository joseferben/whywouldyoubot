...
.PHONY: check.lint
check.lint:
	DJANGO_READ_DOT_ENV_FILE=True python manage.py check --fail-level WARNING
...
.PHONY: coverage
coverage:
	DJANGO_READ_DOT_ENV_FILE=True coverage run -m pytest
	coverage report
...
.PHONY: run
run:
	DJANGO_READ_DOT_ENV_FILE=True python manage.py runserver_plus --nopin
